...
public class PICOVariableAnnotator extends VariableAnnotator {
    ...
    //    @Override
    //    protected void handleClassDeclaration(AnnotatedDeclaredType classType, ClassTree
    // classTree) {
    ...
    protected Slot getOrCreateDeclBound(AnnotatedDeclaredType type) {
        ...
        if (declSlot == null) {
            ...
            // push this change to inference IFF the slot on new class have same requirement with
            // class bound
    ...
    //        // PICO cannot use base extends handling: not simply subtype relationship because of
    // RDM
    ...
    public void storeElementType(Element element, AnnotatedTypeMirror atm) {
        ...
        // TODO make VariableAnnotator::getOrCreateDeclBound protected and override that instead of
        // this method
        if (element instanceof Symbol.ClassSymbol && slot instanceof ExistentialVariableSlot) {
            AnnotationMirror potential =
                    slotManager.getAnnotation(((ExistentialVariableSlot) slot).getPotentialSlot());
    ...
    //    @Override
    //    protected void handleInstantiationConstraint(AnnotatedTypeMirror.AnnotatedDeclaredType
    // adt, VariableSlot instantiationSlot, Tree tree) {
    //        return;
    //    }
    ...
    //        // Forbid any explicit use of @Bottom to be inserted back to source code(no
    // VariableSlot instance is inferred
    //        // @Bottom)
    //        if (generateBottomInequality) {
    //            constraintManager.addInequalityConstraint(varSlot,
    // slotManager.createConstantSlot(BOTTOM));
    //            constraintManager.addInequalityConstraint(varSlot,
    // slotManager.createConstantSlot(POLY_MUTABLE));
    //        }
    //        return varSlot;
    //    }
    ...
    //    @Override
    //    public Void visitWildcard(AnnotatedTypeMirror.AnnotatedWildcardType wildcardType, Tree
    // tree) {
    ...
    //            }
    //            if (!(tree instanceof WildcardTree)) {
    //                throw new IllegalArgumentException("Wildcard type ( " + wildcardType + " )
    // associated " +
    ...
    // extendbound.
    //            ArtificialExtendsBoundTree artificialExtendsBoundTree = new
    // ArtificialExtendsBoundTree(wildcardTree);
