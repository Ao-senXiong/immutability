...
public class PICOInferenceVisitor
        ...
        if (tree instanceof VariableTree && !useType.isDeclaration()) {
            ...
            if (element.getKind() == ElementKind.FIELD
                ...
                AnnotationMirror enclosingBound =
                        extractVarAnnot(
                                PICOTypeUtil.getBoundTypeOfEnclosingTypeDeclaration(
                                        element, atypeFactory));
                ...
                // if enclosing bound == mutable -> (RDM or Mutable usable on mutable-bounded
                // fields)
                // else -> adaptedSubtype
                    ...
                    constraintManager.addImplicationConstraint(
                            Collections.singletonList( // if decl bound is mutable
                                    constraintManager.createEqualityConstraint(
                                            slotManager.getSlot(enclosingBound),
                                            slotManager.getSlot(MUTABLE))),
                            createRDMOnMutableFieldConstraint(
                                    useType, extractVarAnnot(declarationType)));
                ...
                return !AnnotationUtils.areSame(enclosingBound, MUTABLE)
                        || ifAnnoIsThenMainIsOneOf(
                                extractVarAnnot(declarationType),
                                MUTABLE,
                                useType,
    ...
    /**
     * constraint: lhs |> rhs <: lhs equal to decl:immutable => use:immutable || decl:mutable =>
     * use:mutable
     *
     * @param lhs left value of adaption, typically use
     * @param rhs right value of adaption, typically declaration
     */
    private void isAdaptedSubtype(
            AnnotatedTypeMirror lhs, AnnotatedTypeMirror rhs, String msgKey, Tree node) {
        if (extractVarAnnot(lhs).equals(extractVarAnnot(rhs)) || lhs.hasAnnotation(POLY_MUTABLE)) {
            return;
        }
        // todo:haifeng we should do the viewpointAdapt in baseTypeValidator.java#visitDeclared 299
        // function:getTypeDeclarationBounds
    ...
    private Constraint createAdaptedSubtypeConstraint(
        ...
        return constraintManager.createSubtypeConstraint(
                slotManager.getSlot(adapted), slotManager.getSlot(lhs));
    }
    ...
    //        if (!s) {
    //            return
    // atypeFactory.getQualifierHierarchy().isSubtype(ty.getAnnotationInHierarchy(mod), mod);
    //        }
    //        return true;
    //    }
    ...
    public void mainIsSubtype(
        ...
        if (infer) {
            super.mainIsSubtype(ty, mod, msgkey, node);
                ...
                .isSubtype(ty.getAnnotationInHierarchy(mod), mod)) {
    ...
    /**
     * Extract the declaration initialization bound of a certain atm. Return the slot generated
     * during inference.
     *
     * @param atm any AnnotatedDeclaredType
     * @return the initialization bound on the class declaration of the type (actual or slot
     *     annotation)
     */
    private AnnotationMirror extractInitBoundAnno(final AnnotatedDeclaredType atm) {
        Element tm =
                atypeFactory.getProcessingEnv().getTypeUtils().asElement(atm.getUnderlyingType());
        assert tm instanceof TypeElement;
        return extractVarAnnot(
                PICOTypeUtil.getBoundTypeOfTypeDeclaration((TypeElement) tm, atypeFactory));
    }
        ...
        if (infer) {
            ...
            if (element != null
                ...
                if (type instanceof AnnotatedDeclaredType) {
                    ifBoundContainsThenMainIsOneOf(
            ...
            if (element != null
                ...
                // If not use element, but use the atypeFactory.getAnnotatedTypeLhs, anno will
                // refined to initializer's
                ...
                if (type instanceof AnnotatedDeclaredType) {
                    AnnotatedTypeMirror boundType =
                            PICOTypeUtil.getBoundTypeOfTypeDeclaration(
                                    type.getUnderlyingType(), atypeFactory);
                ...
                for (AnnotatedTypeMirror arg : ((AnnotatedDeclaredType) type).getTypeArguments()) {
                    mainIsNoneOf(
                            ...
                            new AnnotationMirror[] {
                                POLY_MUTABLE, BOTTOM, RECEIVER_DEPENDANT_MUTABLE
                            },
    ...
    private void checkAndReportInvalidAnnotationOnUse(AnnotatedTypeMirror type, Tree tree) {
        ...
        if (useAnno != null
            ...
            for (AnnotationMirror anno :
                    atypeFactory.getTypeDeclarationBounds(type.getUnderlyingType())) {
                if (atypeFactory.getQualifierHierarchy().isSubtype(anno, READONLY)
     ...
     */
    private Constraint createRDMOnMutableFieldConstraint(
            AnnotatedTypeMirror mainAtm, AnnotationMirror mutBound) {
        final ConstraintManager constraintManager =
                InferenceMain.getInstance().getConstraintManager();
        final SlotManager slotManager = InferenceMain.getInstance().getSlotManager();
        ...
        return constraintManager.createImplicationConstraint(
                Collections.singletonList(
                        constraintManager.createEqualityConstraint(
                                slotManager.getSlot(mutBound), slotManager.getSlot(MUTABLE))),
                oneOfConst);
    }
    ...
    private Constraint createMainIsMutableOrRdmConstraint(AnnotatedTypeMirror mainAtm) {
        ...
        return constraintManager.createImplicationConstraint(
                Collections.singletonList(
                        constraintManager.createInequalityConstraint(
                                slotManager.getSlot(MUTABLE), slotManager.getSlot(mainAtm))),
                constraintManager.createEqualityConstraint(
     ...
     * @param oneOf
     */
    public boolean ifBoundContainsThenMainIsOneOf(
            AnnotatedDeclaredType atm, AnnotationMirror contains, AnnotationMirror[] oneOf) {

        AnnotationMirror boundAnno = extractInitBoundAnno(atm);
        return ifAnnoIsThenMainIsOneOf(boundAnno, contains, atm, oneOf);
    }

    public boolean ifAnnoIsThenMainIsOneOf(
        ...
        if (this.infer) {
            final ConstraintManager constraintManager =
                    InferenceMain.getInstance().getConstraintManager();
            final SlotManager slotManager = InferenceMain.getInstance().getSlotManager();
            Constraint oneOfConst =
                    constraintManager.createImplicationConstraint(
                            Collections.singletonList(
                                    constraintManager.createInequalityConstraint(
                                            slotManager.getSlot(oneOf[0]),
                                            slotManager.getSlot(mainAtm))),
                            constraintManager.createEqualityConstraint(
                                    slotManager.getSlot(oneOf[1]), slotManager.getSlot(mainAtm)));

            constraintManager.addImplicationConstraint(
                    Collections.singletonList(
                            constraintManager.createEqualityConstraint(
                                    slotManager.getSlot(annotation), slotManager.getSlot(is))),
                    oneOfConst);
            ...
            if (AnnotationUtils.areSameByName(annotation, is)) {
                return AnnotationUtils.containsSameByName(
                        Arrays.asList(oneOf), mainAtm.getAnnotationInHierarchy(READONLY));
            }
        }
        return true;
    }
    ...
    /**
     * Make the main annotation on {@code atm} cannot infer to given {@code anno}. But the written
     * annotation still have effect.
     *
     * <p>A notable use could be poly annotations which could be used by inference if explicitly
     * present, but new poly cannot be inferred.
     *
     * @param atm the type which should not inferred to given anno
     * @param anno the anno that cannot be inferred to
     * @param errorKey this will show only if things goes wrong and result into a error message in
     *     type-check.
     * @param tree this will show only if things goes wrong and result into a error message in
     *     type-check.
     */
    public void mainCannotInferTo(
            AnnotatedTypeMirror atm, AnnotationMirror anno, String errorKey, Tree tree) {
        if (infer) {
    ...
    public Void visitMethod(MethodTree node, Void p) {
        ...
        mainCannotInferTo(
                executableType.getReturnType(), BOTTOM, "type.invalid.annotations.on.use", node);
        if (executableType.getReceiverType() != null) {
            mainCannotInferTo(
                    executableType.getReceiverType(), POLY_MUTABLE, "cannot.infer.poly", node);
        }
        ...
        if (TreeUtils.isConstructor(node)) {
            ...
            if (infer) {
                ...
                constraintManager.addImplicationConstraint(
                        Collections.singletonList(inequalityConstraint), subtypeConstraint);
    ...
    private void flexibleOverrideChecker(MethodTree node) {
        ...
        for (Map.Entry<AnnotatedDeclaredType, ExecutableElement> pair :
            ...
            ((ViewpointAdapterGettable) atypeFactory)
                    .getViewpointAdapter()
                    .viewpointAdaptMethod(
                            enclosingType,
                            pair.getValue(),
                            overriddenMethod); // todo: should we cast it?
    ...
    private void checkInitializingObject(
            ...
            AnnotatedTypeMirror
                    receiverType) { // todo: haifeng we only need to do this in one statement
    ...
    private boolean isInitializingObject(ExpressionTree variable) {
        ...
        // No possibility of initializing object if the assignment is not within constructor or
    ...
    public Void visitMethodInvocation(MethodInvocationTree node, Void p) {
        ...
        if (TreeUtils.isSuperConstructorCall(node)
                && PICOTypeUtil.isAnonymousClassTree(
                        TreePathUtil.enclosingClass(atypeFactory.getPath(node)), atypeFactory)) {
            return null;
        }
    ...
    protected void checkMethodInvocability(
        ...
        if (method.getElement().getKind() == ElementKind.CONSTRUCTOR) {
            ...
            // subtype constraint between the
            // below two types GENERALLY(not always)
            if (!PICOTypeUtil.isEnumOrEnumConstant(subClassConstructorReturnType)
                    && // THIS IS A HECK: java.lang.Enum itself is considered immutable but its
                    // subclasses could be other. Update jdk.astub?
                    !atypeFactory
                            .getTypeHierarchy()
                            .isSubtype(
    ...
    public void processClassTree(ClassTree node) {
        ...
        if (TypesUtils.isAnonymous(TreeUtils.typeOf(node))) {
            checkAnonymousImplements(
                    node, PICOTypeUtil.getBoundTypeOfTypeDeclaration(typeElement, atypeFactory));
    ...
    protected void checkExtendsImplements(ClassTree classTree) {
        ...
        // maybe extract a method between class bound and extends/implements annos and override
        // that.
    }

    /**
     * The base visitor does not use inference method to check! This method is required to add the
     * constraints for extends / implements.
     *
     * @param node
     */
    private void checkAnonymousImplements(ClassTree node, AnnotatedDeclaredType bound) {
        ...
        if (infer) {
            ...
            if (node.getExtendsClause() != null) {
                ...
                assert node.getImplementsClause().size()
                        == 1; // anonymous class only implement 1 interface
            ...
            AnnotationMirror superBound =
                    extractInitBoundAnno(
                            (AnnotatedDeclaredType) atypeFactory.getAnnotatedType(superClause));
     ...
     *
     * <p>Could be subtype, but recall Readonly and Bottom is not usable on class init bound.
     *
     * @param node
     * @param bound
     */
    private void checkSuperClauseEquals(ClassTree node, AnnotatedDeclaredType bound) {
        ...
        for (Tree impTree : node.getImplementsClause()) {
            ...
            boundVsExtImpClause(
                    bound, impType, "declaration.inconsistent.with.implements.clause", impTree);
        }
    }

    private void boundVsExtImpClause(
            AnnotatedDeclaredType classBound,
            AnnotatedTypeMirror superType,
            String errorKey,
            Tree tree) {
        // atypeFactory.getTypeDeclarationBounds does not work correctly: getting the real annos
        // instead of slots
        AnnotatedTypeMirror superBound =
                PICOTypeUtil.getBoundTypeOfTypeDeclaration(
                        superType.getUnderlyingType(), atypeFactory);
    ...
    //    private boolean checkCompatabilityBetweenBoundAndSuperClassesBounds(ClassTree node,
    // TypeElement typeElement, AnnotatedDeclaredType bound) {
    ...
    protected void commonAssignmentCheck(
        ...
        // Removing this check will satisfy initial typecheck of
        // inferrable/issue144/ComplicatedTest.java:42,
        // where invalid.annotations.on.use is not expected.
        // Local variable is flow-sensitive, so when assigned to a type that contradicts with the
        // init bound,
        ...
        if (varTree instanceof VariableTree) {
            ...
            if (element.getKind() == ElementKind.FIELD && !ElementUtils.isStatic(element)) {
                ...
                ((ViewpointAdapterGettable) atypeFactory)
                        .getViewpointAdapter()
                        .viewpointAdaptMember(bound, element, varAdapted);
