...
public class PICOSolverEngine extends SolverEngine {
    ...
    // Collection<Constraint> constraints, QualifierHierarchy qualHierarchy, ProcessingEnvironment
    // processingEnvironment) {
    //        InferenceResult result= super.solve(configuration, slots, constraints, qualHierarchy,
    // processingEnvironment);
    //        if (collectStatistics && result.hasSolution()) {
    //            writeInferenceResult("pico-inference-result.txt",
    ...
    protected SolverFactory createSolverFactory() {
        return new MaxSatSolverFactory() {
            ...
            public MaxSatFormatTranslator createFormatTranslator(Lattice lattice) {
                ...
                // adaptation to underlying solver
