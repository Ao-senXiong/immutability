...
public class PICOInferenceAnnotatedTypeFactory extends InferenceAnnotatedTypeFactory
    ...
    protected TypeAnnotator createTypeAnnotator() {
        // Reuse PICODefaultForTypeAnnotator even in inference mode. Because the type annotator's
        // implementation
        // are the same. The only drawback is that naming is not good(doesn't include "Inference"),
        // thus may be
        // hard to debug
        return new ListTypeAnnotator(
                super.createTypeAnnotator(), new PICODefaultForTypeAnnotator(this));
    }
    ...
    public AnnotatedDeclaredType getSelfType(Tree tree) {
        ...
        // annotations from "methodReceiver"
        return type;
    }
    ...
    public Set<AnnotationMirror> getTypeDeclarationBounds(TypeMirror type) {
        ...
        // This factory is only invoked on inference, so no need to provide concrete anno for
        // type-check
        ...
        //
        //            atm =
        // PICOTypeUtil.getBoundTypeOfTypeDeclaration(TypesUtils.getTypeElement(type), this);
        //        } else {
        //
        ...
        //        }
        AnnotationMirror am =
                ((PICOVariableAnnotator) variableAnnotator)
                        .getClassDeclAnno(getProcessingEnv().getTypeUtils().asElement(type));
        if (am != null) {
        ...
        AnnotatedTypeMirror atm =
                stubTypes.getAnnotatedTypeMirror(getProcessingEnv().getTypeUtils().asElement(type));
        if (atm != null) {
        ...
        return Collections.singleton(
                slotManager.getAnnotation(
                        slotManager.getSlot(
                                super.getTypeDeclarationBounds(type).iterator().next())));
    }
    ...
    class PICOInferencePropagationTreeAnnotator extends PropagationTreeAnnotator {
        ...
        public Void visitMethodInvocation(
                MethodInvocationTree methodInvocationTree, AnnotatedTypeMirror mirror) {
            // This is a workaround for implicit types.
            // Implicit types in lib method get defaulted to mutable.
            // Implicit immutable classes cannot be annotated in stub files, annotations were
            // ignored.
