...
/**
 * The InitializationAnnotatedTypeFactory for the PICO type system. This class is mainly created to override
 * getUninitializedFields() method for PICO specific definite assignment check.
 */
public class PICOInitializationAnnotatedTypeFactory extends InitializationAnnotatedTypeFactory {
    ...
    public List<VariableTree> getUninitializedFields(
        ...
        uninitializedFields.removeIf(
            var -> {
                ...
                AnnotatedTypeMirror.AnnotatedDeclaredType bound =
                        PICOTypeUtil.getBoundTypeOfTypeDeclaration(typeElement, factory);
                // If the class is not annotated with @Immutable or @ReceiverDependentMutable, return false
                if (bound.hasAnnotation(MUTABLE)) {
                    ...
                    if (PICOTypeUtilnested.isAssignableField(varElement,this) || ElementUtils.isStatic(varElement)) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    //TODO this is a hack for calling static class method for lambda expression, consider remove with refactor of PICOTypeUtil
